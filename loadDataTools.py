...
class ParseDFInputParameters(BaseParseData):
    ...
    def __parsePointsParameters(self):
        ...
        loadingPointsTimes = [] # every loading point have more than one time control point
        
        nums = []
        if fps>0:
            while fps>0: # numbers for points may not in one line or all in one line 
                line = self.__file.getNextLine()    
                nums = line.split()
                for i in nums:
        ...
        if lps>0:
            while lps>0:
                line = self.__file.getNextLine()
                nums = line.split()
                loadingPointsTimes.extend(nums)
                lps -= len(nums)
            if lps<0: return False
            
        if len(df_inputDatabase.loadingPoints)>0:
            for i , num in enumerate(loadingPointsTimes):
                loadingPointsTimes[i] =self.parseIntNum(num) 
                if loadingPointsTimes[i]==None :
                    return False
                print 'loading points time points: ', loadingPointsTimes            
            
        # parse loading points parameters  (starttime , stressX , stressY  , endtime , stressX , stressY)
        for i in loadingPointsTimes:
            timeControlPts = []
            for j in range(i):
                digits = [1]*3
                ...
                if None in digits:
                    return False
                timeControlPts.append(digits)
            self.paras.loadingPointMats.append(timeControlPts)
            
            print 'time control point for loadiing point %d : '%i, timeControlPts
    ...
    def __parseRestPara(self ):
        ...
        nums = line.split()
        for i in range(3):
            tmps[i] = self.parseFloatNum(nums[i]) 
            if nums[i] ==None:
                return False
        if tmps[0]==0 and tmps[1]==0 and tmps[2]==0:
            self.paras.ifRightHand = 0
        elif tmps[0]==0 and tmps[1]==1 and tmps[2]==2:
            self.paras.ifRightHand = 1
        print nums
        
        
        # section angle
        line = self.__file.getNextLine()
        if len(line.strip())==0:  # the parameter file is valid if it ends here
            return True
        self.paras.sectionAngle = self.parseFloatNum(line)
        if self.paras.sectionAngle==None:
            return False
        
        # water
        line = self.__file.getNextLine()
        nums = line.split()
        waterPtsNum = self.parseIntNum(nums[0])
        self.paras.waterUnitWeight = self.parseFloatNum(nums[1])
        if waterPtsNum==None or self.paras.waterUnitWeight==None:
            return False
        for i in range(waterPtsNum):
            line = self.__file.getNextLine()
            nums = line.split()
            t1 ,t2 = self.parseFloatNum(nums[0]), self.parseFloatNum(nums[1])
            if t1==None or t2==None:
                return False
            self.paras.waterControlPoints.append((t1,t2))
        print 'water control points : ', self.paras.waterControlPoints
        return True
    ...
    def parse(self , path = None):
        ...
        if self.__parseParaSchema() and self.__parsePointsParameters() \
            and self.__parseBlocksAndJointsPara() and self.__parseRestPara():
#            if self.paras.ifRightHand==1 and not  self.__parseEarthquake():
#                return False
            return True
...
#            print line ,
        ...
        print 'block materials : ', blockMatCollection
