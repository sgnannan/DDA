...
def clearDocument():
    from Base import delaycommand
    delaycommand(_clearDocument)
...
class DCCalculate(DCPreview):
    ...
    def Activated(self):
        ...
        import os
        path = os.path.join(Base.__currentProjectPath__,'data_backup.dc')
...
class DisplayDXFInputData:
    '''
    load dxf input data , and show it.
    '''
    def GetResources(self):
        return {'Pixmap'  :'LoadDXFInput',
                'MenuText':  QtCore.QT_TRANSLATE_NOOP('DDA_ShowDXFInputGraph','DisplayDXFInputData'),
                'ToolTip': QtCore.QT_TRANSLATE_NOOP('DDA_ShowDXFInputGraph',"Display DXF Input Data")}   
...
FreeCADGui.addCommand('DDA_ShowDXFInputGraph', DisplayDXFInputData())   #Ivan, dxf
