...
#        print 'table check result : ' , valid
    ...
    def changeCurrentItem(self):
        ...
        j = self.table.currentColumn()
        if(j==self.getColumnEndIndex()-1):
            j-=1
        else:
            j+=1
    ...
    def clearContent(self):
        for i in range(self.table.rowCount()):
            for j in range(self.getColumnStartIndex() , self.getColumnEndIndex()):
                try:
                    self.table.item(i,j).setText('')
                except:
                    print 'item( %d , %d) clearFaild'%(i,j)
    ...
    def ifEmpty(self):
        ...
        for i in range(self.table.rowCount()):
            ...
            for j in range(self.getColumnStartIndex() , self.getColumnEndIndex()):
                try:
                    if len(self.table.item(i,j).text())>0:
                        empty = False
                        break
                except:
                    print 'item( %d , %d) detect faild'%(i,j)
        return empty
...
class ChooseProjectPath:
    ...
    def Activated(self):
        ...
        if Base.__currentProjectPath__!=None:
            dialog.setDirectory(Base.__currentProjectPath__)
        ...
        if dir:
            ...
            print dir
            Base.setCurrentProjectPath(dir)
            self.saveSettings()
...
class ViewFitAll:
    ...
    '''
    def GetResources(self):
        return {'Pixmap'  :'view-zoom-all',
                'MenuText': QtCore.QT_TRANSLATE_NOOP('DDA_ViewFitAll',"view fit all"),
                'ToolTip': QtCore.QT_TRANSLATE_NOOP('DDA_ViewFitAll',"reset camera to fit whole scene.")}
...
FreeCADGui.addCommand('DDA_ViewFitAll', ViewFitAll())
